...
export function OrderDetailPage() {
  ...
  if (err) return <div className="rounded-2xl bg-red-50 p-4 text-sm text-red-700">{err}</div>
    ...
    <div className="space-y-4">
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          <a href={`/r/${o.orderCode}`} target="_blank" rel="noreferrer" className="rounded-2xl bg-primary-50 px-3 py-2 text-sm font-bold text-primary-600">
            🔗 Resi publik
          </a>
        </div>
        <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="rounded-2xl bg-slate-50 p-2">
            ...
            <span className={remaining > 0 ? 'font-bold text-red-600' : 'font-bold text-turquoise-600'}>{remaining > 0 ? idr(remaining) : 'Lunas ✓'}</span>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <div className="mb-2 flex items-center justify-between">
          ...
          <button className="text-sm font-semibold text-primary-600" onClick={() => location.assign(`/api/t/${slug}/labels.pdf?codes=${o.orderCode}`)}>
            🖨 Cetak label
          </button>
        </div>
              ...
              {it.photos.length > 0 && (
                <div className="mt-2 flex gap-1.5">
                  {it.photos.slice(0, 4).map((p: any) => (
                    <img key={p.id} src={p.url} alt="" className="h-14 w-14 rounded-xl object-cover ring-1 ring-slate-200" />
                  ))}
                </div>
              )}
            </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-4 gap-2">
          {d.photos.filter((p: any) => p.type === 'after').map((p: any) => (
            <img key={p.id} src={p.url} alt="" className="aspect-square rounded-xl object-cover ring-1 ring-slate-200" />
          ))}
          {proof.map((p: any) => (
            <div key={p.id} className="relative">
              <img src={p.url} alt="" className="aspect-square rounded-xl object-cover ring-2 ring-turquoise-400" />
              <span className="absolute bottom-1 left-1 rounded bg-turquoise-600 px-1 py-0.5 text-[9px] font-bold text-white">BUKTI AMBIL</span>
            </div>
          ))}
        </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {o.status !== 'completed' && o.status !== 'abandoned' && (
            <Button className="col-span-2 bg-turquoise-600" onClick={() => setPickupOpen(true)}>
              📦 Konfirmasi Pengambilan
            </Button>
          )}
          {o.status === 'completed' && !o.reviewRequestSentAt && (
            <Button className="col-span-2 bg-pink-500 shadow-pink-500/30" onClick={markReview}>
              ⭐ Minta Review Google Maps
            </Button>
          )}
          {showShare && (
            <Button className="col-span-2 bg-pink-600" onClick={() => setShareOpen(true)}>
      ...
      <Modal open={pickupOpen} onClose={() => setPickupOpen(false)} title="Konfirmasi Pengambilan">
        <div className="space-y-3">
          ...
          <Button
            className="w-full min-h-14 bg-turquoise-600 text-lg"
            loading={busy}
      ...
      <Modal open={shareOpen} onClose={() => setShareOpen(false)} title="Kartu Hasil Before/After">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Button className="min-h-24 flex-col bg-pink-700" onClick={() => share('9:16')}>
            <span className="text-2xl">▯</span>
            Story 9:16
          </Button>
        </div>
